...
/**
 * 任務確認列表（共用元件）
 * 給 pages/TaskConfirmations.jsx 獨立頁 + pages/Approve.jsx 簽核中心 tab 共用
 *
 * @param {Array}   confs       任務確認清單（liff_list_my_task_confirmations 回傳）
 * @param {string}  lineUserId  LINE userId
 * @param {fn}      onReload    成功處理後的 reload callback
 * @param {string}  [emptyText] 空狀態文字
 */
export default function TaskConfirmationList({ confs, lineUserId, onReload, emptyText = '目前沒有等你確認的任務' }) {
  const [processing, setProcessing] = useState(null)
  const [expandedId, setExpandedId] = useState(null)
  const [details, setDetails] = useState({})

  const toggleExpand = async (tc) => {
    if (expandedId === tc.task_id) { setExpandedId(null); return }
    setExpandedId(tc.task_id)
    if (details[tc.task_id]) return
